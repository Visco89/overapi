...
def top_heroes(user, _cat=None, _mode=None):
    ...
    tree = util.base_tree(user);
    keys = tree.xpath(
        '//div[@id="{}"]//div[@data-category-id="{}"]/div/div/div/div[@class="title"]/text()'.format(mode, category))
    values = tree.xpath(
        '//div[@id="{}"]//div[@data-category-id="{}"]/div/div/div/div[@class="description"]/text()'.format(mode,
                                                                                                           category))
    return dict(zip(keys, values))


def career(user, _champ=None, _mode=None):
    if _champ is None:
        champ = CHAMPION["all"]
    else:
        champ = CHAMPION[_champ]
    if _mode is None:
        mode = "quickplay"
    else:
        mode = _mode
